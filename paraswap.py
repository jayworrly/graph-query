...
BATCH_SIZE = 5000  # Smaller batches for rate limiting
RPC_RATE_LIMIT = 5000  # Max requests per batch to stay under 2048 limit
...
class AutoPostgresScanner:
    ...
    def auto_load_tokens(self) -> Set[str]:
        """Automatically load tokens with their activity blocks from bonding_events"""
        target_tokens = set()
        ...
        try:
            ...
            # First, get the optimal block range from bonding_events
            range_query = """
            SELECT 
                MIN(be.block_number) as earliest_activity_block,
                MAX(be.block_number) as latest_activity_block,
                COUNT(DISTINCT be.token_address) as tokens_with_activity,
                COUNT(be.id) as total_bonding_events
            FROM bonding_events be
            WHERE be.block_number IS NOT NULL
            """
            
            logger.info("Analyzing bonding_events for optimal block range...")
            cursor.execute(range_query)
            range_result = cursor.fetchone()
            
            if range_result:
                earliest = range_result['earliest_activity_block']
                latest = range_result['latest_activity_block']
                tokens_count = range_result['tokens_with_activity']
                events_count = range_result['total_bonding_events']
                
                logger.info(f"📊 Bonding Events Analysis:")
                logger.info(f"   Earliest activity: block {earliest:,}")
                logger.info(f"   Latest activity: block {latest:,}")
                logger.info(f"   Tokens with activity: {tokens_count:,}")
                logger.info(f"   Total bonding events: {events_count:,}")
                logger.info(f"💡 RECOMMENDED START_BLOCK: {earliest}")
                
                # Store for later use
                self.recommended_start_block = earliest
                self.activity_block_range = (earliest, latest)
            
            # Now get all tokens that have bonding activity - CORRECTED JOIN
            tokens_query = """
            SELECT DISTINCT 
                td.token_address,
                MIN(be.block_number) as first_activity_block,
            ...
            ORDER BY first_activity_block ASC
            """
            
            logger.info(f"Loading tokens with bonding activity...")
            cursor.execute(tokens_query)
            rows = cursor.fetchall()
            
            for row in rows:
                address = row['token_address']
                first_block = row['first_activity_block']
                events = row['total_bonding_events']
            ...
            logger.info(f"✅ Successfully loaded {len(target_tokens)} tokens with bonding activity")
            
        except Exception as e:
            logger.error(f"Error loading tokens with bonding activity: {e}")
            logger.info("Falling back to all tokens from token_deployments...")

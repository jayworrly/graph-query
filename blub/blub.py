...
def extract_wallet_data(json_file_path: str, output_format: str = 'csv') -> List[Dict[str, str]]:
    """
...
def output_to_csv(data: List[Dict[str, str]], filename: str = 'wallet_data.csv'):
...
def output_to_json(data: List[Dict[str, str]], filename: str = 'wallet_data_extracted.json'):
...
def extract_specific_fields_only(json_file_path: str) -> List[Dict[str, str]]:
    """
...
def main():
    ...
    print("="*60)
    print("WALLET DATA EXTRACTOR - Processes ALL wallets in JSON file")
    print("="*60)
    
    # Example usage
    json_file = input("\nEnter the path to your JSON file containing all wallet data: ").strip()
    
    if not json_file:
        print("No file path provided. Creating sample data for demonstration...")
        # Create a sample JSON file with multiple wallets for testing
        sample_data = [
            {
                "index": 5,
                "address": "0xaccbe4a95776c87b7ec8249bac85e4b524be0c0b",
                "name": "0xChefGoose",
                "social": "https://x.com/0xChefGoose",
                "initial_balance": 1450725.0,
                "initial_balance_usd": 1300.0,
                "total_usd_sold": 6119.35
            },
            {
                "index": 6,
                "address": "0x1234567890abcdef1234567890abcdef12345678",
                "name": "CryptoTrader123",
                "social": "https://x.com/cryptotrader123",
                "initial_balance": 2500000.0,
                "initial_balance_usd": 2250.0,
                "total_usd_sold": 8500.50
            }
        ]
        
        with open('sample_wallet_data.json', 'w') as f:
            json.dump(sample_data, f, indent=2)
        json_file = 'sample_wallet_data.json'
        print("Created sample file with multiple wallets: sample_wallet_data.json")
    ...
    if choice == '1':
        ...
        if minimal_data:
            ...
            df.to_csv('minimal_wallet_data.csv', index=False)
            print("\nMinimal data also saved to: minimal_wallet_data.csv")
    else:
